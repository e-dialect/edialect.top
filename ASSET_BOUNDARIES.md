...
| `src/assets/content.js` | Legacy descriptive content preserved unchanged for product continuity; provenance remains unresolved, and no content/database or alternative-relicensing grant is made. See [`CONTENT_PROVENANCE.md`](./CONTENT_PROVENANCE.md). |
...
根软件许可证只覆盖 e-dialect 有权授权的原创应用代码。`src/assets/content.js` 中的遗留
方言简介为保持产品连续性而原样保留，来源仍未解决，不授予独立内容、数据库或替代
重许可权；详见 [`CONTENT_PROVENANCE.md`](./CONTENT_PROVENANCE.md)。`src/assets/*.png`
与 `public/favicon.png` 属于视觉或品牌资产。这些材料不会因为存放于本仓库或由网站
展示而自动取得软件许可证。重用相关材料前必须另行核实来源、权利、授权及商标边界；
生成构建物或截图也不会为其输入内容产生新的许可。
